...
// OJO: Tu tree decía que el archivo está en docs/ directamente, no en docs/formal/
const sourcePath = path.join(packageRoot, "docs/formal/timestamp-trust-profiles.catalog.json");
const outputPath = path.join(packageRoot, "docs/release/timestamp-trust-profiles.release.json");
...
async function main() {
  ...
  const manifest = {
    ...
    version: 1,
    // Actualizamos esta cadena para que sea coherente con la realidad del repo
    source_catalog: "docs/formal/timestamp-trust-profiles.catalog.json", // CLAVE: debe coincidir con el test
